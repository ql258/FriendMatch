...
import com.ql.friendmatch.model.request.*;
...
//        }
    ...
    public BaseResponse<List<TeamUserVO>> listMyCreateTeams(TeamQuery teamQuery, HttpServletRequest request) {
        ...
        List<TeamUserVO> teamList = teamUserVOList(teamQuery, true);
        return ResultUtils.success(teamList);
    }
...
//        System.out.println("nihaoadasda" + loginUser.getId());
        // 取出不重复的队伍 id
        // teamId userId
        // 1, 2
        // 1, 3
...
//        System.out.println("fdsf" + listMap);
...
//        System.out.println("kdsfaf" + teamList);
        List<TeamUserVO> teamList = teamUserVOList(teamQuery, false);
        return ResultUtils.success(teamList);
    }

    private List<TeamUserVO> teamUserVOList(TeamQuery teamQuery, boolean isAdmin) {
        List<TeamUserVO> teamList = teamService.listTeams(teamQuery, isAdmin);
//        System.out.println("1231" + teamList);
        for (TeamUserVO teamUserVO : teamList) {
            QueryWrapper<UserTeam> teamQueryWrapper = new QueryWrapper<>();
            teamQueryWrapper.eq("teamId", teamUserVO.getId());
            if (!isAdmin) {
                teamUserVO.setHasJoin(true);
            }
            teamUserVO.setHasJoinNum((int) userTeamService.count(teamQueryWrapper));
        }
        return teamList;
    }
    ...
    @ApiOperation("头像上传")
    @PostMapping("/upload")
    public BaseResponse<String> uploadTeam(@RequestPart("file") MultipartFile file) {
        String path = fileService.upload(file);
        if (path == null) {
